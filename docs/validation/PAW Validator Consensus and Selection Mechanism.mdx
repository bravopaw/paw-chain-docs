...
description: Consensus & Selection Mechanism
---
...
The Proposer is a validator with the highest number of stake amounts.
...
3. Take 80% of the validators' potion from the processing fee as the total pool for reward/penalty
4. Distribute the reward and penalty among validators based on their stake percentage.

### *Example Calculation:*

- *Processing Fee = $100*
- *80% of Processing Fee = $80*
...
- *Each Validator's Reward/Penalty Calculation:*
    ...
    - *Reward/Penalty = 80% Processing Fee * Stake Percentage*
...
- The validation result (success or failure) is sent back to the PAW Chain Node.
...
During each transaction, a validator is chosen based on probability rather than randomness. To be eligible as a proposer, a validator must have a minimum stake amount of **1 trillion**. The validator with the highest stake is prioritized as the proposer.
