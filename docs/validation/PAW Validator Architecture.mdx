...
description: PAW Validator Architecture
---
...
On the wallet side, when a sender address is added, the system checks whether the validator is enabled or disabled.

Transactions pass through the validator only if the validator process is enabled. If it is disabled, transactions proceed normally with other validators.
...
(insert validator architecture diagram image)
